...
extension ElasticsearchClient {
    ...
    public func createDocument<Document: Encodable, ID: Hashable>(_ document: Document, with id: ID, in indexName: String) -> EventLoopFuture<ESCreateDocumentResponse<ID>> {
        do {
            let url = try buildURL(path: "/\(indexName)/_doc/\(id)")
            let body = try ByteBuffer(data: self.jsonEncoder.encode(document))
            var headers = HTTPHeaders()
    ...
    public func createIndex(_ indexName: String, mappings: [String: Any], settings: [String: Any]) -> EventLoopFuture<ESDeleteIndexResponse> {
        do {
